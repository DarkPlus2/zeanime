...
export default function Movies() {
  ...
  useEffect(() => {
    let url = `/api/animes?type=movie&sort=${sortOption}`;
  ...
  return (
    <div className="bg-gray-900 text-white min-h-screen">
      ...
      <main className="p-6">
        {movies.length === 0 ? (
          <p className="text-gray-400 text-center mt-12">No movies found.</p>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
            {movies.map((movie) => (
              <Link key={movie.id} href={`/watch/${movie.id}`} className="group relative">
                <img
                  src={movie.cover}
                  alt={movie.title}
